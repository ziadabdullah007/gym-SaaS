...
def check_in(data:AttendanceCheckIn,db:Session=Depends(get_db),gym_id=Depends(get_current_gym_id),_=Depends(require_role(["gym_admin","staff"]))):return AttendanceService.check_in(db,gym_id,data.member_id,data.guests)
