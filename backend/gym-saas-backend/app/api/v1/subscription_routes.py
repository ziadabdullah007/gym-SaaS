...
from app.schemas.subscription_schema import SubscriptionCreate,SubscriptionUpdate,SubscriptionResponse,FreezeCreate
...
def freeze(subscription_id:UUID,data:FreezeCreate,db:Session=Depends(get_db),gym_id=Depends(get_current_gym_id),_=Depends(require_role(["gym_admin"]))):
    obj=SubscriptionService.freeze(db,subscription_id,gym_id,data.days,data.reason)
    return obj
