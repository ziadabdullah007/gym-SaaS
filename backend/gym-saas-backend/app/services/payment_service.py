...
class PaymentService:
    ...
    def create(db, gym_id, data):
        # Idempotency: retrying the same request must not create another payment.
        if data.get("idempotency_key"):
            existing = db.query(Payment).filter(Payment.idempotency_key == data["idempotency_key"]).first()
            if existing:
                if (
                    str(existing.subscription_id) != str(data.get("subscription_id"))
                    or abs(float(existing.amount) - float(data.get("amount", 0))) > 0.009
                ):
        ...
        sub = (
            ...
            .filter(
                ...
                Subscription.member.has(gym_id=gym_id),
            )
            .first()
        )
        if not sub:
        ...
        # A subscription can have any number of payment transactions until its
        # balance reaches zero. Payments are the immutable payment history.
        obj = Payment(
            status="completed",
    ...
    @staticmethod
    def list(db, gym_id):
        return PaymentRepository.get_all(db, gym_id)

    @staticmethod
    def get(db, id, gym_id):
        return PaymentRepository.get_by_id(db, id, gym_id)
