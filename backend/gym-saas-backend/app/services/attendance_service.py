...
class AttendanceService:
    ...
    def check_in(db,gym_id,member_id,guests=None):
        ...
        try:
            ...
            db.add(obj); db.flush()
            for guest in guests:
                db.add(GuestInvitation(
                    gym_id=gym_id,subscription_id=subscription.id,member_id=member_id,attendance_id=obj.id,
                    guest_name=guest.name,guest_phone=guest.phone,guest_age=guest.age,guest_weight=guest.weight,
